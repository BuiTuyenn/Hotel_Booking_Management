...
if($_GET['id']){
	$id = $_GET['id'];
	$qry = $mysqli->query("SELECT * FROM checked where id =".$id);
	if($qry->num_rows > 0){
	...
	if($room_id > 0){
	$room = $mysqli->query("SELECT * FROM rooms where id =".$room_id)->fetch_array();
	$cat = $mysqli->query("SELECT * FROM room_categories where id =".$room['category_id'])->fetch_array();
}else{
	$cat = $mysqli->query("SELECT * FROM room_categories where id =".$booked_cid)->fetch_array();
